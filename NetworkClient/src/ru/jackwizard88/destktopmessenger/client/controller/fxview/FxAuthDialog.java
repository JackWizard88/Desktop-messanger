...
public class FxAuthDialog implements Window {
    ...
    private void onOK() {
        ...
        String saltedPass = clientController.saltPassword(pass);

        if (!loginText.getText().trim().isEmpty() && !passwordText.getText().trim().isEmpty()) {
            try {
                clientController.sendAuthMessage(login, saltedPass);
            } catch (IOException e) {
                showInfoMessage("Ошибка аутентификации. Проверьте данные");
            }
